...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    tags$img(src = 'synapse_logo_blk.png'),
    width = 250,
    sidebarMenu(
    id = "tabs", 
    ...
    box(
      ...
      solidHeader = TRUE,
      width = 12,
      p("ADDING WORDS .... template sync with tab2 "),
  ...
  dashboardBody(
    tags$head(
      tags$style(sass(sass_file("www/styles.scss"), cache = F)),
      singleton(
...
# second tab content
      tabItem(tabName = "data",
              ...
              fluidRow(
                box(
                  ...
                  selectizeInput(inputId = "var", label = "Project:",
                                 choices = "Generating..."),
                  uiOutput('folders'),
                  actionButton("refresh", "", icon = icon("sync-alt")),
              ...
              fluidRow(
                box(status = "primary",
                    solidHeader = TRUE,
                    title = "Completion Plots",
                    plotOutput("stats_plot", height = 250)),
                box(
                  status = "primary",
                  solidHeader = TRUE,
                  title = "List of data type",
...
server <- function(input, output, session) {
  ...
  # table
  set.seed(12)
  df <- data.frame(project=c(''),
                   data_type=c(''), 
                   status=sample(c("Completed", "Not Completed"), 10, replace = T),
                   synapse_url=c(''))
  icons <- ifelse(df$status == "Completed", as.character(icon("check")), as.character(icon("times")))
  df <- cbind(icons, df) %>% rename(` ` = icons)
  output$stats_tbl <- DT::renderDT({
          datatable(df, options = list(lengthChange = FALSE, scrollX = TRUE), escape = FALSE,
            ) %>% formatStyle("status",
                              backgroundColor = styleEqual(c("Completed", "Not Completed"), c("#28a745", "#E53935"))) 
...
output$manifest_sidebar <- renderUI({
  selectInput(inputId = "template_sidebar",
              label = "Current Selected Template:",
