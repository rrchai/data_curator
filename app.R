...
library(shinydashboardPlus) # load after shinydashboard
...
ui <- shinydashboardPlus::dashboardPage(
  ...
  dashboardBody(
    tags$head(
      tags$style(sass(sass_file("www/scss/main.scss"), cache = F)),
      singleton(
              ...
              fluidRow(
               box(
                  ...
                  selectizeInput(inputId = "var", label = "Project:",
                                 choices = "Generating..."),
                  uiOutput('folders'),
                  helpText(HTML("If your recently updated folder does not appear,<br>
                                 please wait for Synapse to sync and click the refresh button"))
                ),
                box(
                  status = "primary",
                  solidHeader = TRUE,
...
server <- function(input, output, session) {
  ...
  # move mouse -> sidebar collapse or not
